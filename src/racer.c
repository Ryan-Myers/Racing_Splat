...
void func_80045C48(Object *obj, Object_Racer *racer, s32 updateRate) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (racer->unk1C9 == 0) {
            ...
            //(sp + i)->unk74 = (f32) temp_v0_2->unk2E[racer->unk1CA];
            //(sp + i)->unk64 = (f32) temp_v0_2->unk32[racer->unk1CA];
    ...
    temp_a0 = -temp_a0;
    var_v1_4 = (temp_v0_5 & 0xFFFF) - (obj->segment.trans.x_rotation & 0xFFFF);
    if (var_v1_4 > 0x8000) {
        var_v1_4 -= 0xFFFF;
    }
    var_v0 = 5;
...
void func_8004CC20(s32 updateRate, f32 updateRateF, Object* racerObj, Object_Racer* racer) {
    ...
    if (racer->boostTimer == 0) {
        racer->boostTimer = 8;
        racer_play_sound(racerObj, 0x107);
    }
    ...
    guMtxXFMF(mtx, 0.0f, 1.0f, 0.0f, &racer->ox2, &racer->oy2, &racer->oz2);
    ...
    if (racer->playerIndex != -1) {
        ...
        if (obj != NULL) {
            ...
            if (sqrtf((xDiff*xDiff) + (yDiff*yDiff) + (zDiff*zDiff)) < 200.0) { 
                ...
                temp = -((racer->ox3 * racerObj->segment.trans.x_position) + (racer->oz3 * racerObj->segment.trans.z_position));
                steerAngle = (((racer->ox3 * obj->segment.trans.x_position) +  (racer->oz3 * obj->segment.trans.z_position)) + (temp));
                steerAngle /= 5;
            }
        }
    }
    ...
    var_f0 = (steerAngle * var_f0) / 360.0f;
    var_f2 = 1.0f;
    ...
    racer->unk1D2 = 0;
    moveObjResult = move_object(racerObj, temp2 * updateRateF, racerObj->segment.y_velocity * updateRateF, racerObj->segment.z_velocity * updateRateF);
    ...
    guMtxXFMF(mtx, racerObj->segment.x_velocity, racerObj->segment.y_velocity, racerObj->segment.z_velocity, &racer->lateral_velocity, &racer->unk34, &racer->velocity);
    second_racer_camera_update(racerObj, racer, 6, updateRateF);
    if ((racerObj->unk60 != NULL) && (racer->vehicleIDPrev == 0) && (racerObj->unk60->unk0 >= 4)) {
